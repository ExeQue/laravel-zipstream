...
> service to a MinIO image you can access. The test only needs an S3-compatible API on port 9000 with the
> credentials above.
...
```bash
ZIPSTREAM_S3_ENDPOINT=http://localhost:9000 vendor/bin/pest
```
...
```bash
ZIPSTREAM_S3_ENDPOINT=http://localhost:9000 vendor/bin/pest tests/Feature/SaveToS3Test.php
```
